...
// MARK: - IDNA Encoding Support
extension String {
    /// Returns the IDNA-encoded version of the string if it contains non-ASCII characters.
    /// This is a simplified implementation that handles basic IDNA encoding.
    public var idnaEncoded: String? {
        // If the string is already ASCII, no encoding is needed
        guard !self.isASCII() else {
            return nil
        }
        
        // For now, we'll return a basic encoded version
        // In a production environment, you might want to use a proper IDNA library
        var encoded = ""
        for scalar in self.unicodeScalars {
            if scalar.isASCII {
                encoded.append(String(scalar))
            } else {
                // Convert non-ASCII characters to their percent-encoded form
                let hexString = String(scalar.value, radix: 16, uppercase: false)
